...
#if (OS_EVENT_EN) && (OS_MAX_EVENTS > 0)
OS_EXT  OS_EVENT         *OSEventFreeList;          //空闲链表指针/* Pointer to list of free EVENT control blocks    */
OS_EXT  OS_EVENT          OSEventTbl[OS_MAX_EVENTS];//全局event数组/* Table of EVENT control blocks                   */
#endif
...
#if OS_TASK_STAT_EN > 0  //cpu统计任务
OS_EXT  INT8U             OSCPUUsage;               /* Percentage of CPU used                          */
OS_EXT  INT32U            OSIdleCtrMax; //1s空闲计数器最大值/* Max. value that idle ctr can take in 1 sec.     */
OS_EXT  INT32U            OSIdleCtrRun; //1s空闲计数器 /* Val. reached by idle ctr at run time in 1 sec.  */
OS_EXT  BOOLEAN           OSStatRdy;    //当前任务状态表示 /* Flag indicating that the statistic task is rdy  */
...
OS_EXT  OS_TCB           *OSTCBCur;      //当前任务tcb指针                  /* Pointer to currently running TCB         */
OS_EXT  OS_TCB           *OSTCBFreeList;  //指向处于空闲态tcb的指针  /* Pointer to list of free TCBs             */
OS_EXT  OS_TCB           *OSTCBHighRdy;                    /* Pointer to highest priority TCB R-to-R   */
OS_EXT  OS_TCB           *OSTCBList;   ///* Pointer to doubly linked list of TCBs    */
OS_EXT  OS_TCB           *OSTCBPrioTbl[OS_LOWEST_PRIO + 1];///* Table of pointers to created TCBs        */
OS_EXT  OS_TCB            OSTCBTbl[OS_MAX_TASKS + OS_N_SYS_TASKS]; //所有任务的数组   /* Table of TCBs                  */
...
#if (OS_MEM_EN > 0) && (OS_MAX_MEM_PART > 0)
OS_EXT  OS_MEM           *OSMemFreeList;           //内存分区管理 /* Pointer to free list of memory partitions       */
...
extern  INT8U   const     OSUnMapTbl[256];         //常数表辅助计算最高优先级 /* Priority->Index    lookup table                 */
