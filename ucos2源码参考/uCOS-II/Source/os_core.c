...
#endif
#if OS_TIME_GET_SET_EN > 0 //时间获取
    //时间单位ticks 发送了多少次timtick的时间
    //读改写因此需要保护
...
//【注释】
//OS_TICK_STEP_EN  使ucos view（内核之外的功能模块，实时监测监控内核便于调试使用，内嵌进到内核中）

#if OS_TICK_STEP_EN > 0                                     /* Enable tick stepping feature for uC/OS-View   */
        //OSTickStepState只是读出来
...
//主体功能  进程调度有关
        ...
        //保护范围越小 实时性越好
            OS_ENTER_CRITICAL();
            //delay函数将延时时间换算成ticks数量，写入到OSTCBDly里面，并将当前任务设置为非就绪态
            //【具体实现】每一次systick中断处理中 将OSTCBDly减1，之后看是否等于0，不等于0说明时间没到，如果到了将当前任务设置为就绪态
            //再次进行调度（如果当前任务是最高的优先级则调度执行）
            if (ptcb->OSTCBDly != 0) {                     /* No, Delayed or waiting for event with TO     */
                if (--ptcb->OSTCBDly == 0) {               /* Decrement nbr of ticks to end of delay       */
                    //对应操作看-1后是否等于0                                       /* Check for timeout                            */
            //为0设置当前任务为就绪态
                    //#define  OS_STAT_PEND_ANY         (OS_STAT_SEM | OS_STAT_MBOX | OS_STAT_Q | OS_STAT_MUTEX | OS_STAT_FLAG)
    //检验阻塞等待
                    if ((ptcb->OSTCBStat & OS_STAT_PEND_ANY) != OS_STAT_RDY) {
						// pend time out
                        //当前tvb有1位 等待时间消除，但是等待时间标志转为OS_STAT_PEND_TO
...
*                            specific.
*				ָ��ջ��ʼλ�õ�ָ��(ARM��һ�����ջ������ΪARMһ�㶼������ջ)
*              pbos          is a pointer to the bottom of stack.  A NULL pointer is passed if called by
*                            'OSTaskCreate()'.
*				ָ��ջ�׵�ָ�룬�����OSTaskCreate()�������ô�NULL�����ˣ������OSTaskCreateEx��ͬ
...
*                            specific.  'stk_size' is 0 if called by 'OSTaskCreate()'.
				stk_size�����õ�ջ�Ĵ�С����λΪջԪ�ظ���(��ջԪ����8λ��λ�����ֽڣ�ջԪ����32λ����4�ֽ�
				)ע�⣬��OSTaskCreate()����ʱֱ�Ӵ�0����
*
...
*                            in this TCB extension.  A NULL pointer is passed if called by OSTaskCreate().
				ָ��һ���û��ṩ���ڴ������׵�ַ������ڴ������������TCB����չ��������������չ������������
				�л�ʱ���渡�㵥Ԫ�Ĵ�����MMU�Ĵ����������κζ������У�������Ҳ���Ը�ÿ���κ��������Ȼ�������
				�ַ����浽���TCB��չ�����ж��С���OSTaskCreate()����ʱֱ�Ӵ�NULL���ɡ�
*
...
*
* Note       : This function is INTERNAL to uC/OS-II and your application should not call it.
				�ú�����ucos2���ڲ�����(Ҳ����˵��ucos2�ڲ�д�õĺ������õ�)�����Լ���Ӧ�ó������
				����Ҫ��Ҳ��Ӧ�õ�����������������о����������Ҫ��Ϊ��ѧϰucos2��ԭ��������TCBʱ
				����������ʲô��������д����ʱҪ�õ���
*********************************************************************************************************
*/
